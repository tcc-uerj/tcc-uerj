...
values
    (nextval('challenge_question_id_seq'), 21, 'Evitando funções longas', 'Em clean code, devemos evitar funções muito ____________, que fazem mais do que uma única tarefa.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 22, 'Princípio do DRY', 'Um dos princípios do clean code é o DRY, que significa Don''t Repeat _______.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 23, 'Nomes significativos', 'Para manter o código limpo, devemos escolher nomes de variáveis, funções e classes que sejam ____________ e descritivos.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 24, 'Mantendo funções abstratas', 'Uma prática comum em clean code é manter as funções com um único nível de ____________.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 25, 'Importância das convenções de formatação', 'Para garantir a legibilidade do código, é importante seguir as ____________ de formatação, como indentação e espaçamento consistente.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 26, 'Reduzindo o uso de comentários', 'Segundo os princípios do clean code, devemos evitar o uso excessivo de ____________.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 27, 'Facilidade de leitura do código', 'Uma das características do código limpo é a facilidade de ____________, o que significa que podemos entender rapidamente o que ele faz.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 28, 'Funções com propósito único', 'Em clean code, é recomendado que cada função tenha um único ____________, fazendo apenas uma coisa e fazendo bem.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 29, 'Evitando acoplamento forte', 'Um dos princípios do clean code é evitar o acoplamento ____________, onde as classes dependem muito umas das outras.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 30, 'Ênfase em testes automáticos', 'O clean code enfatiza a importância de escrever testes ____________, que garantem que o código funcione conforme o esperado.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 31, 'Padrão Factory', 'O padrão ____________ é usado para criar objetos de um tipo específico sem expor a lógica de criação diretamente ao cliente.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 32, 'Organização em árvore hierárquica', 'No padrão ____________, objetos são organizados em uma árvore hierárquica, onde cada nó pode ter zero ou mais nós filhos.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 33, 'Padrão Visitor', 'O padrão ____________ é usado para representar uma operação a ser realizada nos elementos de uma estrutura de objetos.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 34, 'Garantindo uma única instância', 'O padrão ____________ é usado para garantir que um objeto tenha apenas uma instância e fornecer um ponto global de acesso a essa instância.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 35, 'Encapsulando algoritmos intercambiáveis', 'O padrão ____________ é usado para definir uma família de algoritmos, encapsular cada um deles e torná-los intercambiáveis.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 36, 'Fornecendo uma interface unificada', 'O padrão ____________ é usado para fornecer uma interface unificada para um conjunto de interfaces em um subsistema.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 37, 'Encapsulamento de comportamento', 'No padrão ____________, um objeto é usado para encapsular todos os detalhes de um comportamento em particular.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 38, 'Alterando comportamento conforme estado interno', 'O padrão ____________ é usado para permitir que um objeto altere seu comportamento quando seu estado interno muda.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 39, 'Controlando acesso a objetos', 'No padrão ____________, um objeto representa outro objeto para controlar o acesso a ele.', 'GAP_GAME'),
    (nextval('challenge_question_id_seq'), 40, 'Separando lógica de negócios e representação', 'O padrão ____________ é usado para separar a lógica de negócios de uma aplicação de sua representação.', 'GAP_GAME');
